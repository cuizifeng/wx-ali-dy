	...
	<view class="quick-message">
		...
		<view class="quick-message-centre" :style="[messageAniStyle,directionStyle[direction].style]">
			...
			<view class="quick-message-list" :class="['classList' + (index+1)]" v-for="(item,index) in msgList" :key="index">
				<view class="quick-message-list-child" :style="[item.customStyle]"
					:class="[item.type + '-message',item.class?item.class:'']">
					<view class="msg-child-content">
						<image style="width:100rpx;height:100rpx;flex-shrink: 0;" :src="item.img" mode="aspectFit" class="msg-icon"
							v-if="item.img" />
						<block v-else>
							<icon class="msg-icon" :type="item.icon" :size="item.iconSize" :color="item.iconColor?item.iconColor:''"
								v-if="item.icon" />
						</block>
	...
	export default {
		...
		methods: {
			...
			getClass(className) {
				...
				if (box) {
					let data = box.getBoundingClientRect();
					resolve({
						bottom: data.bottom, //: 666
						height: data.height, //: 36
						left: data.left, //: 86
						right: data.right, //: 122
						top: data.top, //: 630
						width: data.width, //: 36
						x: data.x, //: 86
						y: data.y, //: 630

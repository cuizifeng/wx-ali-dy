		...
		methods: {
			...
			setContent(html, append) {
				...
				if (append)
					...
					html =
						'<meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1,minimum-scale=1,maximum-scale=1,user-scalable=no"><style scoped>html,body{width:100%;height:100%;overflow:hidden}body{margin:0}</style><base href="' +
			...
			_handleHtml(html, append) {
				if (!append) {
					...
					var style = '<style scoped>@keyframes _show{0%{opacity:0}100%{opacity:1}}img{max-width:100%}';

...
<template>
  <view>
    <view class="me-tabs" :class="{'tabs-fixed': fixed}" :style="{height: '100rpx', top:topFixed, 'margin-top':topMargin}">
      <scroll-view v-if="tabs.length" :id="viewId" :scroll-left="scrollLeft" scroll-x scroll-with-animation :scroll-animation-duration="300">
        <view class="tabs-item" :class="{'tabs-flex':!isScroll, 'tabs-scroll':isScroll}">
          ...
          <!-- 下划线 -->
          <view class="tabs-line" :style="{left:lineLeft}">抢购中</view>
        </view>
      </scroll-view>
    </view>
  </view>
...
.me-tabs {
  ...
  .tabs-line {
    ...
    line-height: 36rpx;
    text-align: center;
    background: linear-gradient(245deg, #ffc046 0%, #fbae15 100%);
    border-radius: 18rpx;
    font-size: 21rpx;
    font-family: PingFangSC-Regular, PingFang SC;
    font-weight: 400;
